...
function insertLineInJavaClassMethod(
  ...
  if (!classMatch) {
    log.error(`Class ${targetClass} not found.`);
    return javaFileData;
  }
  ...
  if (!methodMatch) {
    log.error(`Method ${targetMethod} not found in class ${targetClass}.`);
    return javaFileData;
  }
  ...
  if (closingBraceIndex === -1) {
    log.error(`Could not find closing brace for method ${targetMethod} in class ${targetClass}.`);
    return javaFileData;
  }
  ...
  if (lineToInsertAfter) {
    ...
    if (lineIndex !== -1) {
      ...
      log.error(`Line "${lineToInsertAfter}" not found in method ${targetMethod} of class ${targetClass}.`);
      return javaFileData;
    }
  }
