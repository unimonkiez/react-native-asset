...
const linkPlatform = ({
  ...
  assetsPaths.forEach((assetMightNotAbsolute) => {
    let asset;
  ...
  Object.keys(linkOptionsPerExt).forEach((fileExt) => {
    const [
      prevAssetsWithExtension,
      assetsWithExtension,
    ] = [
      prevAssets,
      assets,
    ].map(files => files.filter(asset => path.extname(asset) === `.${fileExt}`));

    if (shouldUnlink) {
      if (prevAssetsWithExtension.length > 0) {
        log.info(`Cleaning previously linked ${fileExt} assets from ${name} project`);
        cleanAssets(prevAssetsWithExtension, config, linkOptionsPerExt[fileExt]);
      }
    }

    if (assetsWithExtension.length > 0) {
      log.info(`Linking ${fileExt} assets to ${name} project`);
      copyAssets(assetsWithExtension, config, linkOptionsPerExt[fileExt]);
    }
  });
