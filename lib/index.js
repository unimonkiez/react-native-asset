...
const linkPlatform = ({
  ...
  fileFilters.forEach(({ name: fileConfigName, filter: fileConfigFilter, options }) => {
    ...
    const assetsToRemove = key === undefined
      ? prevRelativeAssetsWithExt
      : prevRelativeAssetsWithExt.filter(({ key: k }) => k === key);
    ...
    if (shouldUnlink && assetsToRemove.length > 0) {
  ...
  const assetsToWrite = key === undefined
    ? assets
    : prevRelativeAssets
      .map(asset => ({ ...asset, path: `${rootPath}/${asset.path}` }))
      .filter(({ path: p, key: k }) => !assets.some(asset => asset.path === p) && k !== key)
      .concat(assets.map(({ key: k, ...asset }) => (
        k === undefined ? asset : { key: k, ...asset }
      )));

  manifest.write(assetsToWrite
...
module.exports = ({
  ...
  [
    ...
    .forEach(linkPlatform({ rootPath, shouldUnlink, key }));
};
