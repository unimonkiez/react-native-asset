...
const { fontTypes, imageTypes } = require('./file-types');
...
const linkPlatform = ({
  ...
  fileFilters.forEach(({ name: fileConfigName, filter: fileConfigFilter, options }) => {
    ...
    if (prevRelativeAndroidFontAssetsToRelink.length > 0) {
      ...
      log.info(null, `Relinking old ${fileConfigName} assets from ${name} project to use XML resources`);
      cleanAssets(
      ...
      copyAssets(
        prevRelativeAndroidFontAssetsToRelink
          .filter(a => !prevRelativeAssetsWithExt.some(b => b.path === a.path))
    ...
    if (shouldUnlink && prevRelativeAssetsWithExt.length > 0) {
      log.info(null, `Cleaning previously linked ${fileConfigName} assets from ${name} project`);
      cleanAssets(
        prevRelativeAssetsWithExt
          .filter(a => !prevRelativeAndroidFontAssetsToRelink.some(b => b.path === a.path))
          .map(({ path: filePath }) => getAbsolute({ filePath, dirPath: rootPath })),
        config,
        options,
      );
    ...
    if (assetsWithExt.length > 0) {
      log.info(null, `Linking ${fileConfigName} assets to ${name} project`);
      copyAssets(
        assetsWithExt
          .map(({ path: assetPath }) => assetPath),
        config,
...
module.exports = ({
  ...
  if (showAndroidRelinkingWarning) {
    log.warn(null, 'The old Android font assets were relinked in order to use XML resources. Please refer to this guide to update your application\'s code as well: https://github.com/unimonkiez/react-native-asset#font-assets-linking-and-usage');
  }
};
